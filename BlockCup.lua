        ...
        if FarmActive then
            for _, Item in pairs(Workspace:GetDescendants()) do
                if Item:IsA("BasePart") then
                    -- Buscar cualquier cosa que parezca bola del evento
                    local Name = string.lower(Item.Name)
                    if string.find(Name, "ball") or string.find(Name, "orb") or string.find(Name, "coin") or string.find(Name, "event") or Item.Color == Color3.new(1, 0.4, 0) then
                        
                        -- ATRAER HACIA TI
                        Item.CFrame = RootPart.CFrame
                        
                        -- Si tiene cuerpo físico, quitarle gravedad para que vuele rápido
                        if Item:FindFirstChild("BodyVelocity") then
                            Item.BodyVelocity:Destroy()
                        end
                        if Item:FindFirstChild("BodyGyro") then
                            Item.BodyGyro:Destroy()
                        end
                    end
                end
            end

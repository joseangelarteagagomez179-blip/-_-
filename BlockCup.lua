...
-- == 2. SISTEMA IMÁN (ESTILO LUXVS) ==
-- Este código detecta CUALQUIER bola del evento como el script original
spawn(function()
    while task.wait(0.05) do -- OPTIMIZADO
        if FarmActive then
            local MyPos = RootPart.Position
            for _, v in pairs(Workspace:GetChildren()) do
                if v:IsA("Part") or v:IsA("MeshPart") then
                    -- Detecta por nombre y propiedades del evento
                    if string.find(string.lower(v.Name), "ball") or string.find(string.lower(v.Name), "orb") or string.find(string.lower(v.Name), "collect") or v.CanCollide == false then
                        
                        local Dist = (MyPos - v.Position).Magnitude
                        if Dist < 60 then -- Rango amplio
                            -- 🧲 ATRACCIÓN FUERTE Y RÁPIDA

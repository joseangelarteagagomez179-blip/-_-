...
-- == 2. IMÁN QUE ATRAE TODAS LAS BOLAS ==
spawn(function()
    while task.wait(0.05) do -- ⚡ OPTIMIZADO: Cada 0.05 segundos es suficiente y no da lag
        if FarmActive then
            local MyPosition = RootPart.Position
            for _, Item in pairs(Workspace:GetChildren()) do
                if Item:IsA("BasePart") and Item.CanCollide == false then -- Solo objetos que se pueden tocar
                    
                    -- 💡 Detecta por nombre y por tamaño (esfera) para agarrar TODAS
                    local Name = string.lower(Item.Name)
                    if string.find(Name, "ball") or string.find(Name, "orb") or string.find(Name, "coin") or string.find(Name, "event") or Item.Shape == Enum.PartType.Ball then
                        
                        local Distance = (MyPosition - Item.Position).Magnitude
                        
                        if Distance < 50 then -- Rango amplio
                            -- 🧲 ATRAER RÁPIDO
                            Item.CFrame = Item.CFrame:Lerp(RootPart.CFrame, 0.8)
